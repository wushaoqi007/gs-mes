...
public class MailFinal {
    ...
    public int getAttachmentsSize() {
        if (CollUtil.isEmpty(this.attachments)) {
            return 0;
        } else {
            return this.attachments.stream().map(attachment -> attachment.getContent().getByteArray().length).reduce(Integer::sum).orElse(0);
        }
    }
}
